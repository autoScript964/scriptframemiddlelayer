 ...
 */
public abstract class AtModule {
    ...
    public AtModule(byte[] data, int offset, int len){
        initField();
        parse(data, offset, len);
    }

    public AtModule(short type){
        initField();
        this.type = type ;
    }
    public int caleSize(){
    ...
    private int caleArraySize(List array){
        ...
        for(int i = 0 ;i < array.size() ; ++i){
            ...
            if(o != null && o instanceof AtModule) {
                size += ((AtModule) o).caleSize() ;
            }else{
...
//    }
    ...
    private boolean putArray(Field field, byte attr, ByteBuffer byteBuffer){
        boolean ret = false ;
        try {
            List<AtModule> list = (List<AtModule>) field.get(this);
            if(attr != 0){
                byteBuffer.put(attr) ;
                byteBuffer.putInt(list.size()) ;
                Iterator<AtModule> iter = list.iterator() ;
                while(iter.hasNext()){
                    AtModule atModule = iter.next() ;
                    atModule.packageData(byteBuffer) ;
                }
                ret = true ;
            }
        } catch (IllegalAccessException e) {
